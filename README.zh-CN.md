...
`book open` 会在 `127.0.0.1:8765` 启动本地服务并打开指定书籍；按下 `Ctrl-C` 可停止服务。阅读器提供章节目录、拖动排序、上一章和下一章、本地全文搜索、浅色和深色主题、阅读位置恢复，以及整本书平均进度。阅读器中还可以新建、编辑或删除书籍，重命名、刷新或删除章节，并通过浏览器历史返回书架。
